...
import { formatUSD, formatReturn, formatTime, pnlColor } from "@/lib/format";
                ...
                <tbody>
                  {visible.map((p, i) => (
                    <tr key={i} className="border-b border-border/50">
                      ...
                      <td className={`py-2 pr-4 ${pnlColor(p.realized_pnl)}`}>
                        {formatUSD(p.realized_pnl)}
                      </td>
                      <td className={`py-2 pr-4 ${pnlColor((p.return || 1) - 1)}`}>
                        {formatReturn(p.return)}
                      </td>
