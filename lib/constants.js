...
export const EVAL_RULES = [
  ...
  { rule: 'Tradable Pairs', parameter: '58 predefined pairs across crypto, commodities, indices, and stocks — see full list on the rules page' },
]

export function getFundedRules(accountType = 'scaled', brandName = 'Hyperscaled') {
  return [
    { rule: 'Daily Loss Limit', parameter: '5% from the day\'s opening equity' },
...
// Weight limits are enforced in three independent layers. A mirrored order
// must satisfy the per-pair, asset-class, and portfolio limit at the same
// time. All limits are a % of your account balance and depend on your Weight
// Tier (see BUYING_POWER_BY_SIZE for the tier that applies to each account).

// Per-pair limit — max exposure to a single pair, by asset class and tier.
export const PER_PAIR_LIMITS = [
  { assetClass: 'Crypto',    a: '50%',  b: '100%', c: '150%' },
  { assetClass: 'Commodity', a: '50%',  b: '100%', c: '150%' },
  { assetClass: 'Index',     a: '150%', b: '300%', c: '450%' },
  { assetClass: 'Stock',     a: '50%',  b: '100%', c: '150%' },
]

// Per-pair overrides — individual pairs tuned above their class default.
// GOLD/USDC is currently the only pair that differs; more may follow.
export const PER_PAIR_OVERRIDES = [
  { pair: 'GOLD/USDC', a: '100%', b: '200%', c: '300%' },
]

// Asset-class limit — max combined exposure across all pairs in one class.
export const ASSET_CLASS_LIMITS = [
  { assetClass: 'Crypto',    a: '200%', b: '200%', c: '300%' },
  { assetClass: 'Commodity', a: '200%', b: '200%', c: '300%' },
  { assetClass: 'Index',     a: '300%', b: '600%', c: '800%' },
  { assetClass: 'Stock',     a: '100%', b: '150%', c: '200%' },
]

// Portfolio limit — max aggregate exposure across all tracked positions.
// Deliberately tighter than the sum of the asset-class limits.
export const PORTFOLIO_LIMITS = { a: '400%', b: '700%', c: '1000%' }
...
export const TRADABLE_PAIRS = [
  {
    category: 'Crypto',
    count: 30,
    pairs: ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'DOGE', 'ADA', 'AVAX', 'LINK', 'DOT', 'TON', 'TRX', 'LTC', 'TAO', 'SUI', 'ARB', 'NEAR', 'ALGO', 'ASTER', 'UNI', 'AAVE', 'CRV', 'HYPE', 'XMR', 'ZEC', 'ENA', 'ZRO', 'WLD', 'PUMP', 'kPEPE'],
  },
  {
    category: 'Commodities',
    count: 6,
    pairs: ['WTIOIL (CL)', 'GOLD', 'SILVER', 'COPPER', 'NATGAS', 'PLATINUM'],
  },
  {
    category: 'Indices',
    count: 3,
...
// the DB. These are legacy list prices — `launchPrice` equals `standardPrice`
// so strikethrough does not render. LunarCrush uses this static fallback.
// Vanta + hyperscaled load STANDARD_TIER_PRICES from the DB via
// `fetchDbPricingTiers("vanta")`.
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: 'Your account equity cannot drop more than 5% from the day\'s opening equity at any point during the trading day. The same 5% daily loss limit applies on both challenge and scaled accounts. The trading day resets at 00:00\u00a0UTC.',
      },
      {
        id: 'eod-trailing',
    ...
    items: [
      {
        ...
        answer: 'The profit target is removed — there\'s no target to hit on scaled accounts. Your daily loss limit stays at 5%, while the EOD trailing loss limit widens to 8% (from 5% during the challenge). You receive monthly USDC payouts based on your realized\u00a0profits.',
      },
      {
        id: 'when-paid',
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: 'You can trade any perpetual available on Hyperliquid. However, only 58 predefined trading pairs across crypto, commodities, indices, and stocks are tracked and counted toward your Hyperscaled trading\u00a0performance.',
      },
      {
        id: 'weight-limits',
        question: 'What are the weight limits?',
        answer: 'Hyperscaled enforces three independent weight limits, all applied automatically: a per-pair limit (max exposure to a single trading pair), an asset-class limit (max combined exposure across all pairs in one class: crypto, commodities, indices, or stocks), and a portfolio limit (max aggregate exposure across all tracked positions). Per-pair and asset-class limits vary by asset class and Weight Tier, and individual pairs can be tuned above their class default (GOLD/USDC currently is). During the challenge, all account sizes use Weight Tier A. On a funded account, $1K\u2013$100K use Tier B, and accounts scaled to $200K\u2013$400K use Tier C. See the rules page for the full per-pair, asset-class, and portfolio limit tables by tier.',
      },
      {
        id: 'trading-fees',
      ...
      },
      {
        ...
        answer: 'There are 58 predefined trading pairs across crypto, commodities, indices, and stocks. Although you can trade any pair on Hyperliquid, only these 58 pairs are tracked and counted toward your Hyperscaled trading\u00a0performance.',
      },
    ],
  },
]
...
// Hyperliquid L1 explorer (registration payments made via the extension's
// "hyperliquid" method or the site's "eip712" usdSend land here, not on Base).
export const HL_EXPLORER_TX_URL = USE_TESTNET
