...
export const LEVERAGE_LIMITS = [
  { tier: 'A', positional: '0.5x', portfolio: '2x' },
  { tier: 'B', positional: '1x',   portfolio: '2x' },
  { tier: 'C', positional: '1.5x', portfolio: '3x' },
]
...
export const FEE_RULES = [
  ...
  { rule: 'Funding Fees', parameter: 'Follows real Hyperliquid settlement mechanics. When the funding rate is positive, longs pay and shorts receive. When negative, longs receive and shorts pay.' },
]
...
// ── WSB Flash Deal Promo ──
//
// Limited-time WallStreetBets x Hyperscaled/Vanta promotion. Applies ONLY to
// the Hyperscaled and Vanta brand variants — all other (white-label) brands
// keep their own pricing. The displayed promo price is always derived from the
// tier's base/standard price so it stays exactly in sync with the discount,
// independent of whatever launch price a brand happens to carry.

export const WSB_PROMO = {
  active: true,
  discountPct: 55,
  endsLabel: 'June 22',
}

// True for the brands that run the WSB flash deal.
export function isWsbBrand(brandId) {
  return brandId === 'hyperscaled' || brandId === 'vanta'
}

// 55% off the base price, rounded up to the nearest whole dollar.
// Returns the input unchanged for free / zero-priced tiers.
export function wsbPromoPrice(basePrice) {
  const base = Number(basePrice)
  if (!Number.isFinite(base) || base <= 0) return basePrice
  return Math.ceil(base * (1 - WSB_PROMO.discountPct / 100))
}
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: 'Hyperscaled expresses leverage as buying power — how much of your account balance you can deploy per trading pair and across the portfolio at once. During the challenge, every account size uses Tier A (0.5x per trading pair, 2x portfolio). On a funded account, $1K\u2013$100K use Tier B (1x per trading pair, 2x portfolio), and accounts scaled to $200K\u2013$400K use Tier C (1.5x per trading pair, 3x portfolio). All limits are enforced automatically by the\u00a0platform.',
      },
      {
        id: 'trading-fees',
        question: 'What fees do I pay to trade?',
        answer: 'Transaction fees are 0.05% (5 bps) per trade. Bid-ask spread, slippage, and funding rates all come directly from Hyperliquid — funding fees follow real Hyperliquid settlement mechanics (when the funding rate is positive, longs pay and shorts receive; when negative, longs receive and shorts pay). No other fees\u00a0apply.',
      },
      {
        id: 'trading-pairs',
