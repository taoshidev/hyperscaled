...
export async function stripAttributionPromoFromCookieValue(rawCookieValue) {
  if (!rawCookieValue || typeof rawCookieValue !== "string") return null;
  const decoded = await verifyAttributionCookie(rawCookieValue);
  if (!decoded?.promo) return null;
  return signAttributionCookie({
    affiliate: decoded.affiliate,
    tenant: decoded.tenant,
    promo: null,
    clickId: decoded.clickId,
    firstTouchAt: decoded.firstTouchAt,
  });
}
