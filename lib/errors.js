...
async function captureWithSentry(error, payload) {
  try {
    const Sentry = await import("@sentry/nextjs");
    Sentry.captureException(error, payload);
  } catch (sentryError) {
    console.error("[errors] Sentry module failed to load", sanitize(sentryError));
  }
}
  ...
  if (isProduction && severity !== SEVERITY.WARNING && typeof window === "undefined") {
    import("@/lib/errors-slack")
      ...
      .catch((e) => {
        ...
        if (isProduction) {
          captureWithSentry(e, { tags: { source: "errors/slack-import" } });
        }
      });
  }
}
...
/**
 * Flush queued Sentry events before a serverless handler returns. Without
 * this, events queued via captureException can be dropped when the Lambda
 * terminates. Safe to call in any runtime — no-ops outside production.
 */
export async function flushErrors(timeoutMs = 2000) {
  ...
  try {
    ...
    console.error("[errors] Sentry flush failed", sanitize(e));
  }
}
