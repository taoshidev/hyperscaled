...
export async function getDb() {
  if (!_db) {
    ...
    const isRemote = !connectionString.includes("127.0.0.1") && !connectionString.includes("localhost");
    ...
    if (process.env.CLOUD_SQL_INSTANCE_CONNECTION_NAME) {
      const { Connector } = await import(/* webpackIgnore: true */ '@google-cloud/cloud-sql-connector');
