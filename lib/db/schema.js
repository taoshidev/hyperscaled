...
export const affiliates = pgTable("affiliates", {
  ...
  /**
   * The promo/coupon code tied to this affiliate (e.g. set by the bulk
   * affiliate import). Used to pre-fill the share-link generator and as the
   * default `?promo=` in exported affiliate links. Not a hard FK — coupons
   * are keyed by their unique code string.
   */
...
export const coupons = pgTable("coupons", {
  ...
  /**
   * True when this coupon was auto-created by the promotional-campaign flow and
   * is therefore exclusively managed by it (its validity window tracks the
   * campaign window). Pre-existing / partner coupons stay `false` and are never
   * mutated by campaign create/update.
   */
  campaignOwned: boolean("campaign_owned").notNull().default(false),
  /** Optional internal description set at creation (command center only). */
  notes: text("notes"),
  /**
   * Optional cohort tag applied when a batch of codes is created together
   * (e.g. the bulk affiliate import). Lets the command center filter and
   * bulk-edit every code from one import at once (e.g. shift the end date).
   */
  batchLabel: varchar("batch_label", { length: 120 }),
  ...
  index("coupons_batch_label_idx").on(table.batchLabel),
]);
