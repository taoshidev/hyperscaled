...
// `priceUsdc` is the *actual* amount charged at registration (the
// `promoPrice` / `launchPrice` on the marketing UI). The slashed/standard
// price displayed alongside it lives in `lib/constants.js` (`TIERS.fullPrice`
// / `PRICING_TIERS.standardPrice`) and `lib/pricing*.js` (`standardPrice`).
//
// Pricing scope:
//   - Vanta (also serves the Hyperscaled brand) → promo prices
//   - Every other miner (Beanstock, Bitcast, LunarCrush, Jolly, Talisman,
//     Zoku, …) → standard prices, no promo
//
// Standard:  $0 / $59 / $109 / $239 / $499 / $799
// Vanta promo: $0 / $29 / $54  / $119 / $249 / $399  (≈ 50% off, rounded down)
...
const MINERS = [
  {
    // Vanta is the only miner on promotional pricing; it also backs the
    // Hyperscaled brand (`app/register/page.jsx` resolves slug "vanta").
    name: "Vanta Trading",
    slug: "vanta",
    hotkeys: {
    ...
    color: "#3b82f6",
    payoutCadenceDays: 7,
    tiers: tiersFor(100, VANTA_PROMO_TIER_PRICES),
  },
  {
