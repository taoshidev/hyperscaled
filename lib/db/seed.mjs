...
const VANTA_USDC_WALLET =
  process.env.NEXT_PUBLIC_HYPERSCALED_USDC_WALLET || null;
if (VANTA_ENTITY_HOTKEY && !VANTA_USDC_WALLET) {
  throw new Error(
    "NEXT_PUBLIC_HYPERSCALED_USDC_WALLET is required when " +
      "VANTA_ENTITY_HOTKEY is set. Set it in .env.local (testnet) or " +
      "your production env (mainnet)."
  );
}
...
const MINERS = [
  ...
  // values must be supplied by the Beanstock operator before going live.
  {
    ...
    usdcWallet: "0x0000000000000000000000000000000000000000",
    apiUrl: "https://api.beanstocktrading.com",
    color: "#22c55e",
