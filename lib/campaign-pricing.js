...
export async function resolveActiveCampaign(opts = {}) {
  ...
  return {
    ...
    // Campaign system unavailable in this environment (e.g. the
    // promotional_campaigns table isn't migrated, or the DB is unreachable at
    // build/prerender time) — degrade gracefully to "no active campaign"
    // instead of throwing and failing the static export. Mirrors the
    // fetchDbPricingTiers fallback.
    console.warn(
      "resolveActiveCampaign: no campaign resolved —",
      err?.message || err,
    );
    return null;
  }
