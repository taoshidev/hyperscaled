...
//                                            brand_variant
//   register_conversion              Account provisioned — final funnel event.
//                                    Params: tier_name, tier_price,
//                                            payment_method, ref_source,
//                                            brand_variant
//   register_conversion_recovered    User refreshed after paying; conversion
//                                    restored from localStorage.
//                                    Params: tier_name, ref_source, brand_variant
//
// `ref_source` defaults to 'direct' if no KOL referral is in sessionStorage
// or URL ?ref= parameter.
...
// Reads the KOL ref source set by <ReferralRedirect>. Checks sessionStorage
// first, then falls back to a ?ref= URL parameter (useful when sessionStorage
// is unavailable, e.g. Safari private mode). Returns 'direct' for organic traffic.
export function getRefSource() {
  if (typeof window === "undefined") return "direct";
  try {
    const stored = window.sessionStorage.getItem("ref");
    if (stored) return stored;
  } catch {
    // sessionStorage unavailable — fall through to URL param check
  }
  try {
