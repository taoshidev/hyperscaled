import crypto from "crypto";
import { getDb } from "./db/index.js";
import { users, registrations } from "./db/schema.js";
import { eq, sql } from "drizzle-orm";

const SUMSUB_BASE_URL = "https://api.sumsub.com";
const APP_TOKEN = () => process.env.SUMSUB_APP_TOKEN;
const SECRET_KEY = () => process.env.SUMSUB_SECRET_KEY;
const WEBHOOK_SECRET = () => process.env.SUMSUB_WEBHOOK_SECRET;
const LEVEL_NAME = () => process.env.SUMSUB_LEVEL_NAME || "basic-kyc-level";

function generateSignature(method, url, ts, body = "") {
  const data = ts + method.toUpperCase() + url + body;
  return crypto
    .createHmac("sha256", SECRET_KEY())
    .update(data)
    .digest("hex");
}

async function sumsubFetch(method, path, body = null) {
  const ts = Math.floor(Date.now() / 1000).toString();
  const bodyStr = body ? JSON.stringify(body) : "";
  const sig = generateSignature(method, path, ts, bodyStr);

  const headers = {
    "X-App-Token": APP_TOKEN(),
    "X-App-Access-Sig": sig,
    "X-App-Access-Ts": ts,
    Accept: "application/json",
  };

  if (body) {
    headers["Content-Type"] = "application/json";
  }

  const res = await fetch(`${SUMSUB_BASE_URL}${path}`, {
    method,
    headers,
    body: body ? bodyStr : undefined,
  });

  return res;
}

export async function createApplicant(externalUserId, email) {
  const path = "/resources/applicants?levelName=" + encodeURIComponent(LEVEL_NAME());
  const payload = { externalUserId };
  if (email) payload.email = email;

  const res = await sumsubFetch("POST", path, payload);
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`SumSub createApplicant failed (${res.status}): ${text}`);
  }
  return res.json();
}

export async function getApplicant(externalUserId) {
  const path = `/resources/applicants/-;externalUserId=${encodeURIComponent(externalUserId)}/one`;
  const res = await sumsubFetch("GET", path);
  if (res.status === 404) return null;
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`SumSub getApplicant failed (${res.status}): ${text}`);
  }
  return res.json();
}

export async function generateAccessToken(externalUserId) {
  const path = `/resources/accessTokens?userId=${encodeURIComponent(externalUserId)}&levelName=${encodeURIComponent(LEVEL_NAME())}`;
  const res = await sumsubFetch("POST", path);
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`SumSub generateAccessToken failed (${res.status}): ${text}`);
  }
  return res.json();
}

/**
 * Reset an applicant back to a re-verification state. Needed because a
 * terminal (FINAL) rejection is sticky: reusing the same externalUserId
 * otherwise re-opens the dead applicant and the SDK immediately shows
 * "Unfortunately, we couldn't verify you", trapping the user forever.
 */
export async function resetApplicant(applicantId) {
  const path = `/resources/applicants/${encodeURIComponent(applicantId)}/reset`;
  const res = await sumsubFetch("POST", path);
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`SumSub resetApplicant failed (${res.status}): ${text}`);
  }
  return res.json().catch(() => ({}));
}

/**
 * True when an applicant carries a terminal (FINAL) RED review — the state
 * that permanently blocks retries until the applicant is reset. Handles both
 * the nested (`review.reviewResult`) and flat (`reviewResult`) shapes SumSub
 * returns across endpoints.
 */
export function isApplicantFinalRejected(applicant) {
  const result =
    applicant?.review?.reviewResult ?? applicant?.reviewResult ?? null;
  if (!result) return false;
  return (
    result.reviewAnswer === "RED" && result.reviewRejectType === "FINAL"
  );
}

export function verifyWebhookSignature(payload, digestHeader, algHeader) {
  const algMap = {
    HMAC_SHA1_HEX: "sha1",
    HMAC_SHA256_HEX: "sha256",
    HMAC_SHA512_HEX: "sha512",
  };
  const hashAlg = algMap[algHeader] || "sha256";
  const hmac = crypto
    .createHmac(hashAlg, WEBHOOK_SECRET())
    .update(payload)
    .digest("hex");
  if (hmac.length !== (digestHeader || "").length) return false;
  return crypto.timingSafeEqual(Buffer.from(hmac), Buffer.from(digestHeader));
}

export async function getUserByWallet(wallet) {
  const db = await getDb();
  const rows = await db
    .select()
    .from(users)
    .where(eq(users.wallet, wallet.toLowerCase()))
    .limit(1);
  return rows[0] || null;
}

export async function createUserByWallet(wallet) {
  const db = await getDb();
  const [newUser] = await db
    .insert(users)
    .values({ wallet: wallet.toLowerCase() })
    .returning();
  return newUser;
}

export async function getAuthorizedWalletsForHlAddress(hlAddress) {
  const db = await getDb();
  const rows = await db
    .select({
      paymentWallet: users.wallet,
      payoutAddress: registrations.payoutAddress,
    })
    .from(registrations)
    .innerJoin(users, eq(registrations.userId, users.id))
    .where(sql`lower(${registrations.hlAddress}) = ${hlAddress.toLowerCase()}`)
    .limit(1);
  if (!rows[0]) return [];
  const wallets = [rows[0].paymentWallet, rows[0].payoutAddress].filter(Boolean);
  return [...new Set(wallets.map((w) => w.toLowerCase()))];
}

export async function updateKycStatus(wallet, fields) {
  const db = await getDb();
  const update = { updatedAt: new Date() };
  if (fields.kycStatus !== undefined) update.kycStatus = fields.kycStatus;
  if (fields.kycApplicantId !== undefined) update.kycApplicantId = fields.kycApplicantId;
  if (fields.kycVerifiedAt !== undefined) update.kycVerifiedAt = fields.kycVerifiedAt;

  const result = await db
    .update(users)
    .set(update)
    .where(eq(users.wallet, wallet.toLowerCase()));

  // node-postgres exposes the affected count as `rowCount`. Callers use this to
  // distinguish "updated a real user" from "matched nobody" (e.g. a webhook
  // event for an externalUserId that isn't a wallet in our DB).
  return result?.rowCount ?? 0;
}
