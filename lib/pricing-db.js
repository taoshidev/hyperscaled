...
import { applyCampaignToTierPrice } from '@/lib/campaign-pricing'
...
const TIER_DISPLAY_META = {
  ...
  5000:   { id: 'tier-1', standardPrice: 74,  scalingPath: 'Up to $400K', popular: false, cta: 'Start $5K Challenge'   },
  10000:  { id: 'tier-2', standardPrice: 135, scalingPath: 'Up to $400K', popular: false, cta: 'Start $10K Challenge'  },
  25000:  { id: 'tier-3', standardPrice: 309, scalingPath: 'Up to $400K', popular: false, cta: 'Start $25K Challenge'  },
  50000:  { id: 'tier-4', standardPrice: 579, scalingPath: 'Up to $400K', popular: false, cta: 'Start $50K Challenge'  },
 ...
 * @param {string} [slug]
 * @param {object} [opts]
 * @param {import('@/lib/campaign-pricing').ActiveCampaign | import('@/lib/campaign-pricing').ActiveCampaignClient | null} [opts.activeCampaign]
 *  When provided, the campaign's per-tier overrides / coupon discount are
 *  applied to the standard list price to derive `launchPrice`. Strikethrough
 *  rendering in PricingPage falls out of `standardPrice > launchPrice`.
 */
export async function fetchDbPricingTiers(slug, opts = {}) {
  const activeCampaign = opts.activeCampaign ?? null
  try {
    ...
    return dbTiers.map((t, i) => {
      ...
      const dbLaunch = Number(t.priceUsdc)
      const listStandard = meta.standardPrice ?? dbLaunch
      const baseStandard = listStandard > dbLaunch ? listStandard : dbLaunch

      const { currentPrice, originalPrice } = applyCampaignToTierPrice(
        t.accountSize,
        baseStandard,
        activeCampaign,
      )
